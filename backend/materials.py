...
NZ retail listings (NZD, GST-inclusive) and are quoted directly in NZD as a
point-in-time snapshot (see `PRICE_SNAPSHOT`). Sizes the retailer does not
list are linearly scaled by cross-section area and marked in `pricing_notes`.
Selecting an engineered product (Prolam/Glulam/LVL) here is a costing choice
only; it is not an engineering substitution — verify against NZS 3604:2011 or
SED.
"""
...
# Point-in-time NZD pricing snapshot (no FX conversion — source prices are NZD).
PRICE_SNAPSHOT = "2026-06-22 09:00 NZST"
PRICE_SNAPSHOT_DATE = "2026-06-22"

DISCLAIMERS = [
    "Prices are indicative supply-only estimates from public NZ retail "
    "listings, quoted in NZD; they are not quotes and exclude delivery, "
...
MATERIALS: dict[str, Material] = {
    "sg8": Material(
        ...
        size_prices_nzd_per_lm={
            "90x45": 6.07, "140x45": 9.44, "190x45": 12.81,
            "240x45": 16.19, "290x45": 19.56,
            "180x25": 6.74, "90x90": 12.14,
        },
    ),
    "sg10": Material(
    ...
    "hyspan": Material(
        ...
        size_prices_nzd_per_lm={
            "90x45": 26.31, "140x45": 40.93,
            "150x45": 43.85, "190x45": 55.54,
            "200x45": 58.51, "240x45": 70.14,
            "300x45": 87.70,
        },
    ),
    "hy90": Material(
        ...
        pricing_notes="hy90 range is 90 mm-thick beams from 150x90 up; the "
                      ...
                      "16.55 NZD/lm).",
        size_prices_nzd_per_lm={
            "90x45": 16.55, "150x90": 55.17,
            "200x90": 75.72, "240x90": 94.73,
            "300x90": 137.36,
        },
    ),
}
