"""Stud material catalogue with sourced NZD-per-lineal-metre pricing.

Estimating data only — NOT quote-grade. Prices are public NZ retail listings
in their native currency, **New Zealand dollars (NZD, GST-inclusive)**, carried
to the valuation date recorded below. No currency conversion is applied: the
catalogue reports the NZD figures directly (v2.2 revised the project from USD
to NZD). Sizes the retailer does not list are linearly scaled by cross-section
area and marked in `pricing_notes`. Selecting an engineered product
(Prolam/Glulam/LVL) here is a costing choice only; it is not an engineering
substitution — verify against NZS 3604:2011 or SED.
"""
...
# Prices are native NZD, so the "FX rate to NZD" is 1.0 (no conversion). The
# fields are retained so the catalogue keeps documenting its valuation basis.
FX_RATE_TO_NZD = 1.0
FX_SOURCE = "native NZD retail pricing — no currency conversion applied"
FX_DATE = "2026-06-23"

DISCLAIMERS = [
    "Prices are indicative supply-only estimates from public NZ retail "
    "listings in NZD; they are not quotes and exclude delivery, fixings, "
    "labour and waste.",
...
class Material:
    ...
    source_price: float
    source_unit: str
    fx_rate_to_nzd: float = FX_RATE_TO_NZD
    fx_source: str = FX_SOURCE
    fx_date: str = FX_DATE
    pricing_notes: str = ""
    # per-size NZD/lm map; sizes not listed fall back to default_nzd_per_lm
    size_prices_nzd_per_lm: dict[str, float] = field(default_factory=dict)


def _nzd(nzd_per_lm: float) -> float:
    """Native NZD per lineal metre, rounded (no FX conversion in v2.2+)."""
    return round(nzd_per_lm, 2)
...
MATERIALS: dict[str, Material] = {
    "sg8": Material(
        ...
        size_prices_nzd_per_lm={
            "90x45": _nzd(6.07), "140x45": _nzd(9.44), "190x45": _nzd(12.81),
            "240x45": _nzd(16.19), "290x45": _nzd(19.56),
            "180x25": _nzd(6.74), "90x90": _nzd(12.14),
        },
    ),
    "sg10": Material(
    ...
    "hychord": Material(
        ...
        size_prices_nzd_per_lm={
            "90x45": _nzd(19.04), "140x45": _nzd(29.62),
            "190x45": _nzd(40.20),
        },
    ),
    "hyspan": Material(
        ...
        size_prices_nzd_per_lm={
            "90x45": _nzd(26.31), "140x45": _nzd(40.93),
            "150x45": _nzd(43.85), "190x45": _nzd(55.54),
            "200x45": _nzd(58.51), "240x45": _nzd(70.14),
            "300x45": _nzd(87.70),
        },
    ),
    "hy90": Material(
        ...
        pricing_notes="hy90 range is 90 mm-thick beams from 150x90 up; the "
                      ...
                      "16.55 NZD/lm).",
        size_prices_nzd_per_lm={
            "90x45": _nzd(16.55), "150x90": _nzd(55.17),
            "200x90": _nzd(75.72), "240x90": _nzd(94.73),
            "300x90": _nzd(137.36),
        },
    ),
}
...
def unit_price_nzd_per_lm(material_key: str,
                          size: str) -> tuple[float, str, str, str, str]:
    """(nzd_per_lm, confidence, source_name, source_url, pricing_notes).
    ...
    if (pm := _SIZE_PREFIX.match(base)):
        mult = int(pm.group(1))
        base = pm.group(2)
    per_lm = m.size_prices_nzd_per_lm.get(base, m.default_nzd_per_lm)
...
def catalogue_json() -> dict:
    return {
        ...
        "currency": "NZD",
        "fx": {"currency": "NZD", "rate_to_nzd": FX_RATE_TO_NZD,
